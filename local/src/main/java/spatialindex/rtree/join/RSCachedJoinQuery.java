            ...
            try {
                for (int j = 0; j < nl.getEntries().size(); j++) {
                    RTreeIEntryData entryNL = (RTreeIEntryData) nl.getEntries()
                            .get(j);
                    List<Geometry> desmataPolygons = ProbabilisticGeometriesService.getLayer1CachedPolygons(entryNL.getChild(), entryNL.getPolygon(), iterations);
                    
                    for (int k = 0; k < nr.getEntries().size(); k++) {
    
                        RTreeIEntryData entryNR = (RTreeIEntryData) nr
                                .getEntries().get(k);
                        List<Geometry> vegetaPolygons = ProbabilisticGeometriesService.getLayer2CachedPolygons(entryNR.getChild(), entryNR.getPolygon(), iterations);
